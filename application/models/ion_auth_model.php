...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		{
			$this->add_to_group($default_group->group_id, $id);
		}
	...
	public function get_user_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['user_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['user_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['user_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.group_id')
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		if ($this->db->where(array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id))->get($this->tables['user_groups'])->num_rows()) return false;

		if ($return = $this->db->insert($this->tables['user_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id)))
		{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			{
				$this->db->delete($this->tables['user_groups'], array($this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id));
		...
		{
			if ($return = $this->db->delete($this->tables['user_groups'], array($this->join['users'] => (int)$user_id))) {
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->db->where($this->tables['groups'].'.group_id', $id);
		}
